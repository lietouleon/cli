...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--file` | 是 | 本地图片路径，**必须是 CWD 内的相对路径**（如 `./pic.png`）。**>20MB 自动走 multipart 分片上传**，无需额外参数 |
...
`+media-upload` 内部调用 `POST /open-apis/drive/v1/medias/upload_all`（>20MB 时走 `upload_prepare` / `upload_part` / `upload_finish` 三步），固定使用：
